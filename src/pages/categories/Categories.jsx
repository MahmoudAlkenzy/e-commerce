...
export default function Categories() {
  ...
  const { categorySubCategories, isLoading: isGetting, mutate } = useCategorySubCategories();
      ...
      <Container>
        <Row className="g-4">
          {categories.map((categ, idx) => {
            return (
              <Col md={idx === 0 ? 12 : 4} className={`${idx < 0 ? 'offset-4' : ''}`} key={idx}>
                {/* <Link to={`/Category/${categ.slug}`}> */}
                <div
                  className={`${idx === 0 ? 'col-4 m-auto ' : ''}  rounded  overflow-hidden`}
                  onClick={() => {
                    mutate({ id: categ._id });
                  }}
                >
                  <img className="w-100 " src={categ.image} alt={categ.name} style={{ height: '300px' }} />
                  <h2 className="h4 my-3 text-center">{categ.name}</h2>
                </div>
                {/* </Link> */}
              </Col>
            );
