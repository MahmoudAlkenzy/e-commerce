      ...
      <div className="product rounded-3 position-relative  h-100 overflow-hidden">
        ...
        <Link to={`/productDetails/${product.id}`}>
          <div className="overflow-hidden ">
            <img style={{ minHeight: '260px', width: '105%' }} src={product.imageCover} alt={product.title} />
          </div>

          <div className="p-2 pb-0 ">
            <h3 className="h6 text-main fw-semibold">{product.category.name}</h3>
            <h2 className="h5 fw-semibold">{product?.title?.split(' ').splice(0, 2).join(' ')}</h2>

            {product.priceAfterDiscount ? (
              <p className="fs-6">
                <span className="fs-6 fw-medium text-main">{formatCurrency(product.priceAfterDiscount)}</span>
                <span className="mx-1 fs-7 text-black-50 text-decoration-line-through fst-italic">
                  {'$' + product.price}
                </span>
              </p>
            ) : (
              <p className="fw-medium  mb-0">{formatCurrency(product.price)}</p>
            )}
          </div>
        </Link>
